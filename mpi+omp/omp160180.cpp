...
void exchange_boundaries(std::vector<double>& grid, const Neighbors& neighbors, MPI_Comm& comm, Boundary& bond, std::vector<double>& send_col, std::vector<double>& send_row,  int& M, int& N) {
...
void solve_iter(std::vector<double>& a, std::vector<double>& b, std::vector<double>& w, std::vector<double>& r, int& dx, int& dy, int& offx, int& offy, Boundary& ba, Boundary& bb, Boundary& bw) {
    #pragma omp parallel for collapse(2)
    for (int i = 0; i < dy; ++i) {
        for (int j = 0; j < dx; ++j) {
...
double scal_prod(std::vector<double>& a, std::vector<double>& b, int& dx, int& dy) {
    double sum = 0;
...
void w_iter(std::vector<double>& w, std::vector<double>& r, double tau, int& dx, int& dy) {
    #pragma omp parallel for collapse(2)
    for (int i = 0; i < dy; ++i) {
        for (int j = 0; j < dx; ++j) {
