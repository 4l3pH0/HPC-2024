...
int main (int argc, char** argv) {
    ...
    double* send_br_top = new double[cdx]();
    double* send_br_bottom = new double[cdx]();
    double* send_br_left = new double[cdy]();
    double* send_br_right = new double[cdy]();
    ...
    if (neighbors.left >= 0) {
        ...
        MPI_Isend((send_row), cdy, MPI_DOUBLE, neighbors.left, 0, MPI_COMM_WORLD, &requests0[request_count++]);
        MPI_Irecv((ba_left), cdy, MPI_DOUBLE, neighbors.left, 1, MPI_COMM_WORLD, &requests0[request_count++]);   
    }
    ...
    if (neighbors.right >= 0) {
        ...
        MPI_Isend(send_row, cdy, MPI_DOUBLE, neighbors.right, 1, MPI_COMM_WORLD, &requests0[request_count++]);
        MPI_Irecv((ba_right), cdy, MPI_DOUBLE, neighbors.right, 0, MPI_COMM_WORLD, &requests0[request_count++]);
    ...
    if (neighbors.top >= 0) {
        ...
        MPI_Isend(send_col, cdx, MPI_DOUBLE, neighbors.top, 2, MPI_COMM_WORLD, &requests0[request_count++]);
        MPI_Irecv((ba_top), cdx, MPI_DOUBLE, neighbors.top, 3, MPI_COMM_WORLD, &requests0[request_count++]);
    }
    ...
    if (neighbors.bottom >= 0) {
        ...
        MPI_Isend(send_col, cdx, MPI_DOUBLE, neighbors.bottom, 3, MPI_COMM_WORLD, &requests0[request_count++]);
        MPI_Irecv((ba_bottom), cdx, MPI_DOUBLE, neighbors.bottom, 2, MPI_COMM_WORLD, &requests0[request_count++]);
    ...
    if (neighbors.left >= 0) {
        ...
        MPI_Isend((send_row), cdy, MPI_DOUBLE, neighbors.left, 0, MPI_COMM_WORLD, &(requests01[request_count++]));
        MPI_Irecv((bb_left), cdy, MPI_DOUBLE, neighbors.left, 1, MPI_COMM_WORLD, &(requests01[request_count++]));
    }
    ...
    if (neighbors.right >= 0) {
        ...
        MPI_Isend(send_row, cdy, MPI_DOUBLE, neighbors.right, 1, MPI_COMM_WORLD, &(requests01[request_count++]));
        MPI_Irecv((bb_right), cdy, MPI_DOUBLE, neighbors.right, 0, MPI_COMM_WORLD, &(requests01[request_count++]));
    }
    ...
    if (neighbors.top >= 0) {
        ...
        MPI_Isend(send_col, cdx, MPI_DOUBLE, neighbors.top, 2, MPI_COMM_WORLD, &(requests01[request_count++]));
        MPI_Irecv((bb_top), cdx, MPI_DOUBLE, neighbors.top, 3, MPI_COMM_WORLD, &(requests01[request_count++]));
    }
    ...
    if (neighbors.bottom >= 0) {
        ...
        MPI_Isend(send_col, cdx, MPI_DOUBLE, neighbors.bottom, 3, MPI_COMM_WORLD, &(requests01[request_count++]));
        MPI_Irecv((bb_bottom), cdx, MPI_DOUBLE, neighbors.bottom, 2, MPI_COMM_WORLD, &(requests01[request_count++]));
    ...
    #pragma acc data copy(cdx, cdy, send_br_top[0:cdx],send_br_bottom[0:cdx], send_br_left[0:cdy], send_br_right[0:cdy], N, M, offx, offy, send_col[0:cdx], send_row[0:cdy],  w[0:cdx * cdy], r[0:cdx * cdy], Ar[0:cdx * cdy], operator_a[0:cdx * cdy], operator_b[0:cdx * cdy], operator_f[0:cdx * cdy], ba_top[0:cdx], ba_bottom[0:cdx], ba_left[0:cdy], ba_right[0:cdy], bb_top[0:cdx], bb_bottom[0:cdx], bb_left[0:cdy], bb_right[0:cdy], bw_top[0:cdx], bw_bottom[0:cdx], bw_left[0:cdy], bw_right[0:cdy], br_top[0:cdx], br_bottom[0:cdx], br_left[0:cdy], br_right[0:cdy])
    while(error > stop){
        ...
        for (int i = 0; i < cdy; ++i) {
            for (int j = 0; j < cdx; ++j) {
                ...
                if (i == 0) {
                    send_br_bottom[j] = r[j];
                }
                if (j == 0) {
                    send_br_left[i] = r[i*cdx];
                }
                if (i == cdy - 1) {
                    send_br_top[j] = r[i*cdx + j];
                }
                if (j == cdx - 1) {
                    send_br_right[i] = r[i*cdx + j];
                }
        ...
        #pragma acc update self(send_br_left[0:cdy], send_br_right[0:cdy], send_br_top[0:cdx], send_br_bottom[0:cdx])
        
        MPI_Request requests1[8];
        request_count = 0;

        if (neighbors.left >= 0) {
            MPI_Isend((send_br_left), cdy, MPI_DOUBLE, neighbors.left, 0, MPI_COMM_WORLD, &(requests1[request_count++]));
            MPI_Irecv((br_left), cdy, MPI_DOUBLE, neighbors.left, 1, MPI_COMM_WORLD, &(requests1[request_count++]));

            
        }

        if (neighbors.right >= 0) {
            MPI_Isend(send_br_right, cdy, MPI_DOUBLE, neighbors.right, 1, MPI_COMM_WORLD, &(requests1[request_count++]));
            MPI_Irecv((br_right), cdy, MPI_DOUBLE, neighbors.right, 0, MPI_COMM_WORLD, &requests1[request_count++]);
            
        }
        if (neighbors.top >= 0) {
            MPI_Isend(send_br_top, cdx, MPI_DOUBLE, neighbors.top, 2, MPI_COMM_WORLD, &(requests1[request_count++]));
            MPI_Irecv((br_top), cdx, MPI_DOUBLE, neighbors.top, 3, MPI_COMM_WORLD, &(requests1[request_count++]));
        }
        if (neighbors.bottom >= 0) {

            MPI_Isend(send_br_bottom, cdx, MPI_DOUBLE, neighbors.bottom, 3, MPI_COMM_WORLD, &(requests1[request_count++]));
            MPI_Irecv((br_bottom), cdx, MPI_DOUBLE, neighbors.bottom, 2, MPI_COMM_WORLD, &(requests1[request_count++]));
        ...
        #pragma acc update device(br_left[0:cdy],br_right[0:cdy],br_top[0:cdx],br_bottom[0:cdx])
        ...
        for (int i = 0; i < cdy; ++i) {
            for (int j = 0; j < cdx; ++j) {
                ...
                if (i == 0) {
                    bw_bottom[j] -= tau * br_bottom[j];
                }
                if (j == 0) {
                    bw_left[i] -= tau * br_left[i];
                }
                if (i == cdy - 1) {
                    bw_top[j] -= tau * br_top[j];
                }
                if (j == cdx - 1) {
                    bw_right[i] -= tau * br_right[i];
                }
            }
        }
