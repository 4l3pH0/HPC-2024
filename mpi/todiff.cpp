...
std::vector<double> gen_grid(double start, double delta, int size) {
...
struct Operator {
    std::vector<double> operator_a;
    std::vector<double> operator_b;
    std::vector<double> operator_f;
    Grid* grid;

    Operator(const std::vector<double>& operator_a,
             const std::vector<double>& operator_b,
             const std::vector<double>& operator_f,
             Grid* grid)
...
Operator* do_op(Grid* grid, int dx, int dy) {
    std::vector<double> operator_a(dx*dy, 0);
    std::vector<double> operator_b(dx*dy, 0);
    std::vector<double> operator_f(dx*dy, 0);
    for (int i = 0; i < dy; ++i) {
        for (int j = 0; j < dx; ++j) {
...
void exchange_boundaries(std::vector<double>& grid, const Neighbors& neighbors, MPI_Comm& comm, Boundary& bond, std::vector<double>& send_col, std::vector<double>& send_row,  int& M, int& N, int& i, int& j) {
...
double choose_wisely(std::vector<double>& a, int i, int j, int dx, int dy, Boundary& b) {
    if (i < 0) {
        return b.bottom[j];
    }
    if (i >= dy) {
...
void solve_iter(std::vector<double>& a, std::vector<double>& b, std::vector<double>& w, std::vector<double>& r, int& dx, int& dy, int& i, int& j, int& offx, int& offy, Boundary& ba, Boundary& bb, Boundary& bw) {
...
void scal_prod(std::vector<double>& a, std::vector<double>& b, int& dx, int& dy, double& sum, int& i, int& j) {
    sum = 0;
    for (i = 0; i < dy; ++i) {
        for (j = 0; j < dx; ++j) {
...
void w_iter(std::vector<double>& w, std::vector<double>& r, double tau, int& dx, int& dy, int& i, int& j) {
    for (i = 0; i < dy; ++i) {
        for (j = 0; j < dx; ++j) {
...
int main (int argc, char** argv) {
    ...
    printf("rank: %d   l: %d r:   %d t:   %d b:   %d\n", rank, neighbors.left, neighbors.right, neighbors.top, neighbors.bottom);
    ...
    std::vector<double> w(cdx*cdy + 1, 0);
    std::vector<double> r(cdx*cdy + 1, 0);
    std::vector<double> Ar(cdx*cdy + 1, 0);
    
    std::vector<double> send_row(cdy);
    std::vector<double> send_col(cdx);
    ...
    exchange_boundaries(A->operator_a, neighbors, comm, ba, send_col, send_row, cdx, cdy, i, j);
    exchange_boundaries(A->operator_b, neighbors, comm, bb, send_col, send_row, cdx, cdy, i, j);

    double start = MPI_Wtime();
    while(error > stop){
        exchange_boundaries(w, neighbors, comm, bw, send_col, send_row, cdx, cdy, i, j);
        solve_iter(A->operator_a, A->operator_b, w, r, cdx, cdy, i, j, offx, offy, ba, bb, bw);
        ...
        exchange_boundaries(r, neighbors, comm, br, send_col, send_row, cdx, cdy, i, j);


        solve_iter(A->operator_a, A->operator_b, r, Ar, cdx, cdy, i, j, offx, offy, ba, bb, br);

        scal_prod(r, r, cdx, cdy, scals[0], i, j);
        scal_prod(Ar, r, cdx, cdy, scals[1], i, j);    

        MPI_Allreduce(scals, pretau, 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        tau = pretau[0] / pretau[1];
        w_iter(w, r, tau, cdx, cdy, i, j);
